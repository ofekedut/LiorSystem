...
router = APIRouter(
    prefix="/person_marital_statuses",
    tags=["person_marital_statuses"]
)
...
async def read_person_marital_statuses():
    conn = await get_connection()
    try:
        rows = await conn.fetch("SELECT id, name, value FROM person_marital_statuses")
...
async def read_person_martial_status(status_id: uuid.UUID):
    conn = await get_connection()
    try:
        row = await conn.fetchrow("SELECT id, name, value FROM person_marital_statuses WHERE id = $1", str(status_id))
        if row is None:
...
async def create_person_martial_status(payload: PersonMaritalStatusInCreate):
    ...
    try:
        row = await conn.fetchrow(
            "INSERT INTO person_marital_statuses (id, name, value) VALUES ($1, $2, $3) RETURNING id, name, value",
...
async def update_person_martial_status(status_id: uuid.UUID, payload: PersonMaritalStatusInUpdate):
    conn = await get_connection()
    try:
        existing = await conn.fetchrow("SELECT id, name, value FROM person_marital_statuses WHERE id = $1", str(status_id))
        if existing is None:
            raise HTTPException(status_code=404, detail="Person marital status not found")
        ...
        updated = await conn.fetchrow(
            "UPDATE person_marital_statuses SET name = $1, value = $2 WHERE id = $3 RETURNING id, name, value",
...
async def delete_person_martial_status(status_id: uuid.UUID):
    conn = await get_connection()
    try:
        result = await conn.execute("DELETE FROM person_marital_statuses WHERE id = $1", str(status_id))
        if result.split()[-1] == "0":
            raise HTTPException(status_code=404, detail="Person marital status not found")
    finally:
        await conn.close()
